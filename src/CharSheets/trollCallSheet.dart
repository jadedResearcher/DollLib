...
class TrollCallSheet extends CharSheet {
    ...
    TrollCallSheet(Doll doll) : super(doll) {
        ...
        name = new TextLayer("Name",nameForDoll(),345.0,470.0, fontSize: 66, maxWidth: 100, fontName: "trollcall", emphasis: emphasis,fontColor: color);
    ...
    String randomFact() {
        ...
        List<String> templates = <String>["Master of the $noun","Practices ${verb}ing the ${noun} every day","Been in detention their whole life","Imitation $noun bracelet","sweet ${noun}s","13 ${noun}s at last count","Sleeps once a week","Does not $verb $noun","Aspiring $noun","Always reblogs $noun posts","Keeps hydrated","Omnicidal except towards ${noun}s","Has a soft spot for $effect ${noun}s","Really into ${noun}s","Probably $effect when you aren't looking","Actually a sweetheart","Ask them about $effect ${noun}s if you can spare an hour","is currently $effect","Never ${verb}s. Ever."];
        templates.addAll(<String>["Has ${verb}ed 13 easter eggs. Yes, even the $effect one.","Is a close personal friend of the Heiress.","Is $effect.","Tries to hide how $effect they are.","${verb}s once a week","Owns a small $noun","Thinks everybody is $effect","Thinks all $effect ${noun}s should be arrested","Never $effect","Always $effect","Wakes up early to $verb ${noun}s","Thinks you can never have too much ${noun}s","Collects antique ${noun}s","Addicted to love.","Has filled all their quadrants.","Has ${verb}ed all their quadrantmates.","Tries to push their moiral to $verb ${noun}s"]);
